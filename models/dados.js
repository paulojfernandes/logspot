...
exports.dadosAtividadesRegisto = function (req, res) {
    ...
    connection.query('SELECT  titulo,tipo_atividade,ls_empresa.nome,DATE_FORMAT(dia_realizacao, "%m/%d/%Y %H:%i") as data, cidade from  ls_atividade,ls_tipo_atividade,ls_empresa,ls_agenda,ls_localizacao where  ls_atividade.id_tipo_atividade = ls_tipo_atividade.id_tipo_atividade AND ls_atividade.id_empresa = ls_empresa.id_empresa   AND ls_atividade.id_agenda = ls_agenda.id_agenda  AND ls_atividade.cod_postal = ls_localizacao.cod_postal AND date(ls_agenda.dia_realizacao)= curdate();  ', function (err, rows, fields) {
        if (!err) {
            console.log("entrei");
            res.send(rows)
        } else {
